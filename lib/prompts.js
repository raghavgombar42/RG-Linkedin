...
- Do not fill in what was said, discussed, seen or felt beyond the note. If the note says "discussing unit economics", do not invent which metrics came up.
- Do not speculate about causes or explanations the note doesn't give (e.g. "probably a packing error"). An honest "no idea why" is fine.
- If a detail is needed but missing, write [VERIFY: what is missing]. It is better to leave a placeholder than to guess.
- The example lines in the voice skill come from Raghav's past posts. Learn the style from them, never reuse them word for word.
